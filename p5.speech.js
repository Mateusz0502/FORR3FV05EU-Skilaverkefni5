...
var myRec = new p5.SpeechRec('en-US', parseResult); // nýr P5.SpeechRec object (valdi ensk til almennari notkunar)
	myRec.continuous = true; // gera stöðuga viðurkenningu
	myRec.interimResults = true; // leyfa hluta viðurkenningu (hraðari, ónákvæmari)
	...
	function setup()
	{
		...
		textSize(20);
		textAlign(LEFT);
		text("draw: up, down, left, right", 20, 20);

		//myRec.onResult = parseResult; // núna í smiðnum
		myRec.start(); // ræsa forrit
	}
	...
	{
		// viðurkenningarkerfi mun oft sameina orð í orðasambönd
		// aðferðin er að nota aðeins síðasta orðið sem sagt var:
